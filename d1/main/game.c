...
int mark_player_path_to_segment_partime(int segnum, int check_id)
{
	int i;
	int j;
	int n;
	...
	int id; // For differentiating between the IDs and indexes of Point_segs.
	int other_id; // For differentiating between the IDs and indexes of walls.
	...
	if (check_id == 1) {
		for (i = 0; i < player_path_length; i++) {
			other_id = lockedWallIDs[i];
			if (Point_segs[i].segnum == Walls[other_id].segnum) {
				id = Point_segs[i].segnum;
				for (j = 0; j < MAX_SIDES_PER_SEGMENT; j++) {
					if (Segments[id].children[j] == Walls[other_id].sidenum) {
						lockSegs++;
						if (Walls[other_id].type == WALL_DOOR) {
						...
						if (Walls[other_id].flags == WALL_DOOR_LOCKED) {
							// We need the unlockID to be the segment of the trigger. I don't yet know exactly how this will be accomplished. EXCLUDE THE EXIT.
						}
						if (lockSegs > 1)
							return unlockID;
					}
			...
			if (Segments[i].special == SEGMENT_IS_ROBOTMAKER) {
				for (j = 0; j < player_path_length; j++) {
					if (i == Point_segs[j].segnum) {
						...
						matcenHealth += 0; // Find the highest robot HP in this matcen and increment by that. 0 is a placeholder.
					}
				}
			}
			return matcenHealth;
